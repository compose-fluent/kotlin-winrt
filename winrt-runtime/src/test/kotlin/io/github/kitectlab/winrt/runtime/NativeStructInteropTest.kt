...
class NativeStructInteropTest {
    ...
    fun native_interop_reads_and_writes_float_and_char16() {
        NativeInterop.confinedScope().use { scope ->
            val floatSlot = NativeInterop.allocateBytes(scope, NativeStructScalarKind.FLOAT32.sizeBytes)
            NativeInterop.writeFloat(floatSlot, 1.5f)
            assertEquals(1.5f, NativeInterop.readFloat(floatSlot))

            val charSlot = NativeInterop.allocateBytes(scope, NativeStructScalarKind.CHAR16.sizeBytes)
            NativeInterop.writeChar16(charSlot, 'K')
            assertEquals('K', NativeInterop.readChar16(charSlot))
        }
    }
    ...
    fun common_geometry_and_numerics_adapters_round_trip() {
        NativeInterop.confinedScope().use { scope ->
            val pointMemory = NativeInterop.allocateBytes(scope, Point.Metadata.layout.sizeBytes)
            val point = Point(1.5f, 2.5f)
            Point.Metadata.copyTo(point, pointMemory)
            assertEquals(point, Point.Metadata.fromAbi(pointMemory))

            val matrixMemory = NativeInterop.allocateBytes(scope, Matrix3x2.Metadata.layout.sizeBytes)
            val matrix =
                Matrix3x2(
                    m11 = 1f,
                    m12 = 2f,
            ...
            val planeMemory = NativeInterop.allocateBytes(scope, Plane.Metadata.layout.sizeBytes)
