...
1. Superpowers development workflows are opt-in for this repository. Unless the user explicitly requests one for the current task, do not require or follow a Superpowers-specific `PLAN -> RED -> Coding -> GREEN` sequence.
2. Follow this repository's reference-first and design-first direction: inspect the matching `.cswinrt` implementation, define the owning Kotlin contract, implement the coherent slice, and then use targeted tests as validation. A deliberately failing RED test is not a prerequisite for implementation.
3. Do not add production abstractions, scaffolding, duplicate tests, or other code solely to satisfy Superpowers process ceremony. Avoid workflow-driven code bloat and prioritize direct, efficient implementation of the `.cswinrt`-aligned design.
4. Temporary documents generated by a Superpowers workflow must not be included in commits. This includes ad hoc implementation plans, RED/GREEN logs, review reports, scratch notes, and similar process artifacts. Remove them or leave them untracked before committing unless the user explicitly asks for a specific document to become permanent repository documentation.
5. The root `PLAN.md` is the repository's canonical planning file and remains subject to the Planning File Rules below; it is a user-approved plan, not a progress log.
6. An explicit user request to use a Superpowers workflow applies only to the task for which it was requested and does not change the repository default for later tasks.
...
5. Before writing a new test for a new slice, identify the matching `.cswinrt` source area and encode that mapping in code comments or task notes. Do not change `PLAN.md` for this purpose unless the user has approved the plan revision.
...
10. If a phase is blocked, record the missing prerequisite in task or module notes and continue with the earliest incomplete prerequisite rather than jumping ahead to a later module. Update `PLAN.md` only under the Planning File Rules below.
...
Do not claim that Kotlin modules are aligned with `.cswinrt` unless the top-level module ownership above is explicitly implemented and the remaining gaps are called out in an approved `PLAN.md` entry or in task/module notes.
...
1. Maintain the root-level `PLAN.md` as the canonical, concise implementation plan.
2. `PLAN.md` contains approved scope, dependency order, stable constraints, and the status of existing implementation items. It must not contain command transcripts, timestamps, benchmark samples, rejected-experiment history, progress diaries, or other log-like content; `PLAN.md` 不允许插入日志性内容。
3. Any change to an item's wording, scope, order, grouping, dependency, addition, removal, split, merge, or structure requires explicit user approval. A direct approval in the current task counts; a code change alone does not. 条目正文或结构的修改必须经过用户明确同意。
4. Without that approval, an agent may modify only the status of an existing item. Use `[x]` for completed items, `[ ]` for incomplete items, `doing:` for active work, and `skipped:` for intentionally skipped work; preserve the item's wording, links, ordering, and surrounding structure. Changing the checkbox marker from `[ ]` to `[x]` is allowed only when completing the item. 未经用户同意，Agents 只允许修改既有条目的完成状态（`doing`、`[x]`、`[ ]`、`skipped`）。
5. Keep Markdown task-list syntax for implementation items. Write completed items as `- [x] ...`, incomplete items as `- [ ] ...`, active items with the `doing:` prefix, and skipped items with the `skipped:` prefix.
6. A status change must reflect the actual repository state. Do not add explanatory prose, measurements, command output, or retrospective detail while changing a status.
7. New implementation slices, scope changes, and missing prerequisites belong in task or module notes until the user approves a corresponding `PLAN.md` revision.
8. Approved entries must preserve the phase order and, where applicable, identify the matching `.cswinrt` responsibility and Kotlin owner.
9. Keep the current-focus queue short and keep explicitly deferred work separate from active work; do not use either section as a work journal.
