...
abstract class RestoreWinAppDependenciesTask : DefaultTask() {
    ...
    fun restore() {
        ...
        val restoreWorkspace = temporaryDir.toPath().resolve("workspace")
        GradleFileOperations.deleteDirectory(restoreWorkspace)
        Files.createDirectories(restoreWorkspace)
        try {
            GradleFileOperations.copyFile(config, restoreWorkspace.resolve("winapp.yaml"))
            ...
            Files.move(restoredOutput, output)
        } finally {
            GradleFileOperations.deleteDirectory(restoreWorkspace)
        }
    }
