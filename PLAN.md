...
- [ ] `winrt-generator` remains in `.cswinrt/src/cswinrt` parity follow-through: the latest audit leaves runtime-class object method override rules active, while server activation/module factory/user-subclassing rules stay authoring-gated until `winrt-authoring` is unfrozen.
...
- [ ] Generator audit 22.16 正在做: align `.cswinrt` `write_class_method` language object method rules; metadata-provided `ToString`, object `Equals`, and `GetHashCode` must render as Kotlin `toString`, `equals`, and `hashCode` overrides backed by the WinRT ABI call instead of ordinary PascalCase methods or only suppressing the pointer-identity fallback.
