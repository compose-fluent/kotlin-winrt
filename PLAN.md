...
- [ ] Server/host activation 正在做: JVM host bridge mirrors `.cswinrt/src/Authoring` `DllGetActivationFactory`/`DllCanUnloadNow` HRESULT semantics, generated support registers module factories before forwarding, library identity/staging handles authored WinMD, and generation writes a JVM authoring host manifest with activatable classes; next is actual native DLL export/packaging.
