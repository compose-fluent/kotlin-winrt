...
- [ ] 24.9: collection owner cache closure: projected collection runtime classes that expose `MutableList`/`MutableMap` must route collection delegates through the correct generated collection interface cache instead of assuming `_defaultInterface`.
- [ ] 24.10: ABI marshaler closure: align non-string `IReference<T>`, `Map.Entry` adapters, object/interface/delegate returns, nullable ABI pointers, collection return values, and adapter construction with `.cswinrt` `abi_marshaler`.
- [ ] 24.11: generic/interface helper initialization: align generic delegate arity, derived generic interface impl classes, generic ABI helper init, event generic dependencies, and fallback helper registration.
...
- [ ] `write_class`, `write_static_class`, `write_class_members`, `write_static_members`: class shells and inherited default-interface caches mostly exist; collection owner cache, base constructor/object-reference routing, and wrapper closure still need Queue 24.
- [ ] `write_custom_mapped_type_members` and required mapped helpers: collections and `IClosable` are partly aligned; bindable/data-error helper routing, helper-only metadata, and generic collection adapters still need Queue 24.
