...
- [ ] Generator ABI 8.21: implement `.cswinrt` `write_generic_type_instantiations` as executable Kotlin support, not descriptor-only output: generated entries must initialize transitive dependencies, bind RCW helper functions, vtable functions, property accessors, and delegate CCW invoke hooks where the JVM runtime can support them.
