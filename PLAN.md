...
- [ ] Authoring WinMD emission 正在做: authored runtime-class descriptors are now owned by `winrt-metadata`, merge into the projection model before source generation, emit a deterministic authoring metadata descriptor file, and have a minimal PE/CLI WinMD shell writer validated by the existing metadata loader; next step is filling TypeDef/InterfaceImpl/custom-attribute tables to match `.cswinrt/src/Authoring/WinRT.SourceGenerator/WinRTTypeWriter.cs`.
