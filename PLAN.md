...
- [ ] Server/host activation 正在做: JVM host bridge mirrors `.cswinrt/src/Authoring` `DllGetActivationFactory`/`DllCanUnloadNow` HRESULT semantics, generated support registers module factories before forwarding, library identity/staging handles authored WinMD and host manifests, and generation writes activatable-class host manifests; next is actual native DLL export/packaging.
