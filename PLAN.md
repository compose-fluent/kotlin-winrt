...
- [ ] Kotlin appx/msix packaging completeness 正在做: finish manifest generation/validation, appx/msix layout, dependency payload resolution, resources/PRI/MRT, signing/test-install hooks, unpackaged vs packaged modes, and Gradle DSL ergonomics without cloning full MSBuild; current slice makes explicit signing and test-install failures fail closed, validates signing input package presence, reports signtool/PowerShell startup failures through stable Gradle diagnostics, and keeps install package input selection provider-based.
