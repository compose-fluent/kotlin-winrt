...
- [x] Closed KWINRT-043 nullable struct setter boxing: generated `IReference<T>` setter IID expressions now use full struct field signatures, and `winrt-runtime` registers built-in `Point`, `Size`, and `Rect` value boxing metadata so nullable WinRT struct values expose the expected `IReference<T>` / `IReferenceArray<T>` interfaces.
