...
- [ ] Server/host activation 正在做: JVM host bridge mirrors `.cswinrt/src/Authoring` `DllGetActivationFactory`/`DllCanUnloadNow` HRESULT semantics, generated support exposes stable host-shim address entrypoints, library identity/staging handles authored WinMD and cswinrt-style activatable-class target host manifests, staged/plugin-runtime-assets manifests install as activation-factory fallbacks, and host loading now uses manifest target artifacts like cswinrt shim target-assembly loading; next is actual native DLL export/packaging.
