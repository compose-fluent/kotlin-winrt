...
- [x] Compiler support TSV schema fail-closed closure: reject extra columns in compiler-support, projection registrar, generic instantiation, and generic ABI registry TSV rows instead of folding generator/schema drift into the last parsed field.
- [x] Compiler support merge schema fail-closed closure: reject malformed dependency compiler-support manifest rows during Gradle merge instead of forwarding drifted support rows into merged compiler-plugin inputs.
- [x] Compiler support merge header fail-closed closure: reject dependency compiler-support manifests whose header no longer matches the generator-owned schema before Gradle merge performs positional row parsing.
...
- [x] Authoring TypeDetails collection element metadata fail-closed closure: reject authored collection return elements whose metadata is missing or whose type reference lacks a qualified name at the adapter boundary instead of collapsing them into a generic unsupported adapter result.
- [x] Authoring TypeDetails object return fail-closed closure: reject authored object returns whose runtime-class default interface or interface IID metadata is missing instead of falling back to `IID.IInspectable` for generated CCW return values.
- [x] Authoring TypeDetails object parameter fail-closed closure: reject authored complex parameters whose metadata is missing or not an object-wrap shape instead of falling back to `IID.IInspectable` metadata wrapping for unsupported parameter kinds.
...
- [x] Authoring host manifest fail-closed closure: reject generated or dependency host manifests without assembly identity or activatable classes instead of silently treating malformed authoring handoff data as no host build input.
...
- [x] Collection and async projection closure: finish generated collection, observable/bindable, iterable/vector/map, async action/operation/progress surfaces, helper support, and runtime bridge calls after runtime contracts are complete.
