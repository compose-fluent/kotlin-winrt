...
- [ ] Narrow first slice 正在做: migrate non-generic interface/runtime-class method and property projection shape first; current implementation emits ordinary common interfaces plus JVM-side projection support objects instead of `expect`/`actual` interfaces, and emits simple non-generic runtime-class `expect`/JVM `actual` wrappers with matching RCW constructors that forward through the full public interface inheritance closure; interface/runtime-class forwarding covers inherited interface member forwarding with duplicate redeclaration suppression guarded by conflict checks, runtime-class member rows covered by inherited non-conflicting public interfaces with normalized raw interface names, matching member return/property types, method parameter names/types, no-exception HRESULT behavior, and property getter/setter shape; eligibility now reuses the computed interface proxy closure for each decision point and requires member public ABI kinds to stay inside the current scalar/string/GUID/object/runtime-class boundary; unsupported ABI call plans, non-FFM ABI shapes, projected interface ABI members, event-bearing types, static-member types, setter-only-property projection types, async surfaces, and collection/custom mapped surfaces stay on the common fallback path instead of failing or silently dropping members during actual generation, while events, delegates, collections, async, structs, enums, activation, projected-interface ABI members, and generic closures stay on the existing path until each slice has target-specific call coverage.
- [ ] JVM ABI call ownership 正在做: JVM actual non-generic interface member bodies now route supported pointer/scalar ABI shapes through generated Java FFM private `JvmAbi.invoke_*` helpers with per-function-address downcall caches, including scalar combinations that were never practical as fixed `ComVtableInvoker.invokeArgs` overloads; simple runtime-class actual wrappers delegate member calls to those interface projections, and generated JVM actual output no longer contains a hidden `ComVtableInvoker` fallback for selected migrated members.
...
- [ ] Expect/actual generator slice 正在做: run `./.agent_scripts/run_windows_gradle.sh --no-daemon --no-build-cache --no-configuration-cache :winrt-generator:test --tests KotlinProjectionGeneratorTest.generator_can_emit_expect_common_and_jvm_actual_interface_slice --tests KotlinProjectionGeneratorTest.expect_actual_interface_native_projection_implements_inherited_interface_members --tests KotlinProjectionGeneratorTest.generator_can_emit_expect_common_and_jvm_actual_runtime_class_slice --tests KotlinProjectionGeneratorTest.expect_actual_interface_slice_falls_back_for_async_and_collection_shapes --tests KotlinProjectionGeneratorTest.expect_actual_runtime_class_slice_falls_back_when_public_interface_uses_async_or_collections --tests KotlinProjectionGeneratorTest.generator_emits_jvm_ffm_for_scalar_shape_without_comvtable_overload`.
