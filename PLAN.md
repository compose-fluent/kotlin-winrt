...
- [ ] Authoring writer output 正在做: generated support now covers metadata mapping, wrapper/ABI facades, custom QI, server factories, module registration, CCW method tables, arrays, and generic-parameter marshaling; next is generated host bridge handoff.
- [ ] Server/host activation 正在做: mirror `.cswinrt/src/Authoring` `DllGetActivationFactory`/`DllCanUnloadNow` semantics as a JVM host bridge first, with actual native DLL export/packaging still tracked separately.
