...
- [x] Generator fail-closed baseline: current generator rejects missing or incomplete metadata for composable factories, activation/static interfaces, default/implemented/required interfaces, event delegates, delegate/runtime-class/interface ABI bindings, unsupported ABI shapes, enum/struct ABI metadata, delegate `Invoke` ABI metadata, and static setter ABI bindings before rendering.
...
- [ ] ABI array contract closure: verify arrays always carry exactly one renderable element ABI binding before projection rendering, including nested generic/array cases and diagnostics that fail before marshaler fallback paths.
