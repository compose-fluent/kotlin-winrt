...
- [x] Generated projection registrar: generator support output now emits `WinRTProjectionRegistrar.register()` with direct projection class literals, runtime-class `Metadata.register()` calls, and structured type-index handoff; JVM runtime loads that fixed registrar first and keeps the TSV path only as compatibility fallback.
...
- [x] Generated projection registrar: targeted generator and runtime registry tests validate generated class-literal registrar output and JVM fixed-entrypoint loading before TSV fallback.
...
- [x] WinUI/sample validation: targeted `winrt-samples:compileKotlin`, sample tests, and manual smoke runs have validated generated projection consumption, WinUI activation, XAML resource setup, window/page/control creation, click/tapped handlers, object collection insertion, and message-loop survival. Broad WinUI smoke remains validation-only and must not drive upstream design.
