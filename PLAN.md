...
- [ ] Gradle cache contract 正在做: keep configuration cache and build cache enabled for WinRT generation and packaging tasks; cache failures are issues to fix, not flags to bypass. Application package staging is cacheable with output-affecting resource-root path modeled as an input; runtime asset staging now snapshots explicit `runtimeAsset(...)` files and dependency-identity runtime asset files, but still needs NuGet package contents modeled before the task can be safely marked cacheable.
