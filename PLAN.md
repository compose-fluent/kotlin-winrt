...
- [ ] Gradle cache contract 正在做: keep configuration cache and build cache enabled for WinRT generation and packaging tasks; cache failures are issues to fix, not flags to bypass. Application package staging must be cacheable with output-affecting resource-root path modeled as an input; runtime asset staging needs a separate NuGet/cache input model before it can be marked cacheable.
