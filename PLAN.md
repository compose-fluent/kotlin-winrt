...
- [ ] `winrt-generator` remains in `.cswinrt/src/cswinrt` parity follow-through: server activation/module factory/user-subclassing rules stay authoring-gated until `winrt-authoring` is unfrozen.
...
- [ ] Queue 22 正在做: close the remaining `.cswinrt/src/cswinrt/code_writers.h` generator rules before treating WinUI sample output as validation; open non-authoring gaps are 22.18 non-blittable struct ABI, 22.19 delegate ABI shape coverage, and 22.20 generic/static ABI helper parity. Server activation/CCW remains authoring-gated in 22.14.
...
- [x] Generator ABI 8.16: `.cswinrt` `write_custom_mapped_type_members` parity is closed for the current Kotlin runtime path: mapped collection/bindable/`IClosable`/`INotifyDataErrorInfo` member families carry structured public/private, explicit-helper, and static-ABI vs IDIC call-mode handoffs; generated runtime-class members use the runtime helper path while static-ABI-only refinements now belong to ABI implementation work.
...
- [x] Generator ABI 8.18: fast-ABI static ABI class folding from `.cswinrt` `write_static_abi_classes` is closed for generated slot ownership: default and exclusive interface companions carry the same folded default-plus-other interface slot sequence, and runtime-class calls route fast-ABI exclusive members through the folded default-interface cache.
...
- [x] Generator ABI 8.19d: `.cswinrt` `write_class_objrefs_definition` generic-interface cache initialization now calls generated `WinRTGenericTypeInstantiations.initializeBySourceType(...)` instead of a no-op lambda; the support file owns deterministic dependency traversal and initialized-entry state, while concrete RCW/vtable helper binding remains tracked in 8.21.
- [x] Generator ABI 8.19: close `.cswinrt` object-reference cache rules from `write_class_objrefs_definition`; manual bindable skips, fast-ABI exclusive skips, sealed `_inner` reuse, default-interface objref dispatch, and generic-interface initialization now all flow through generated object-reference plans.
- [x] Generator ABI 8.20: projected `Activatable`/`Composable` constructors remain active, while `.cswinrt` `write_factory_class`/`write_module_activation_factory` server activation data is deferred authoring handoff only and no longer exposed through active module activation installers.
- [x] Generator ABI 8.21a: generated `WinRTGenericTypeInstantiations` now mirrors `.cswinrt` `Init()` as executable dependency-ordered runtime binding calls for RCW helpers, vtable functions, property accessors, and delegate CCW invoke hooks instead of only recording descriptors.
- [x] Generator ABI 8.21b: `WinRTGenericTypeInstantiations` installs a default runtime binding into `winrt-runtime` so generated generic instantiation initialization records RCW helper, vtable, property accessor, and delegate CCW invoke hook families at the shared runtime boundary.
- [x] Generator ABI 8.21c: generic instantiation writer descriptors now match `.cswinrt` `write_generic_type_instantiation` vtable binding ownership: delegate instantiations bind `Do_Abi_Invoke`, while non-delegate generic interfaces only register RCW/property helper functions.
- [x] Generator ABI 8.21d: generic instantiation dependency descriptors now substitute the current instantiation arguments before recording implemented-interface and event delegate initialization dependencies, matching `.cswinrt` `push_generic_args` behavior.
- [x] Generator ABI 8.21e: generic instantiation worklist now follows `.cswinrt` fixed-point discovery by adding concrete dependency instantiations discovered while writing each instantiation, so generated support does not drop dependency initialization entries.
- [x] Generator ABI 8.21: `.cswinrt` `write_generic_type_instantiations` parity is closed for the current Kotlin runtime boundary: generated entries use fixed-point discovery, initialize transitive concrete dependencies, and bind RCW helper, property accessor, delegate CCW invoke, and delegate-only vtable hook families through `winrt-runtime`; event source helper bodies remain the separate 8.22 slice.
- [x] Generator ABI 8.22a: generated `WinRTEventProjectionHelpers.installEventSources()` is now callable runtime setup, initializing each event delegate generic instantiation and registering event source descriptors in `winrt-runtime` instead of leaving event source data as passive metadata only.
- [x] Generator ABI 8.22b: `EventHandler<T>` event source descriptors now follow `.cswinrt` shared `EventHandlerEventSource<T>` behavior instead of pretending to generate unique `_EventSource_*` subclasses.
- [x] Generator ABI 8.22c: non-`EventHandler<T>` event helper descriptors now install callable `WinRtEventSourceFactory` entries and generated `_EventSource_*` subclasses that bridge `EventSource/EventSourceState` to projected delegate invoke bodies.
- [x] Generator ABI 8.22d: generated event projection support now exposes a table-style `createEventSource(eventType, ownerType, objectReference, slot)` entry backed by `WinRtEventSourceRuntime`, matching `.cswinrt` event source table responsibility before owner member wiring.
- [x] Generator ABI 8.22e: shared `EventHandler<T>` event source descriptors now generate callable `EventHandlerEventSource<T>` factories with parameterized IID/signature and delegate value kind derived from the same generator ABI classification path.
- [x] Generator ABI 8.22f: generated runtime-class and native interface-proxy event properties now prefer the generated event source table and fall back to direct add/remove token methods, while `WinRtEvent` can wrap `EventSource<T>` for handler-based subscribe/unsubscribe parity.
- [x] Generator ABI 8.22g: generated static event properties now follow `.cswinrt` `write_abi_event_source_static_method_call` by acquiring the static interface object and routing subscription through the generated event source table before falling back to direct add/remove token methods.
- [x] Generator ABI 8.22: `.cswinrt` event source helper subclass parity is closed for the current Kotlin runtime boundary: non-`EventHandler<T>` helpers, shared `EventHandler<T>` factories, generated event source table registration/creation, runtime-class events, native interface-proxy events, and static events all route through callable runtime/helper code.
...
- [x] Generator audit 22.15: aligned `.cswinrt` `write_class_members`/`write_class_method`/`write_class_event` rules for `OverridableAttribute` and `ProtectedAttribute`; generated runtime classes now suppress protected/overridable interfaces from the public superinterface list and emit their bound class members as `protected`/`protected open` instead of public `override`, preserving public overrides for ordinary interfaces.
- [x] Generator audit 22.16: aligned `.cswinrt` `write_class_method` language object method rules; metadata-provided `ToString`, object `Equals`, and `GetHashCode` now render as Kotlin `toString`, `equals`, and `hashCode` overrides backed by the WinRT ABI call instead of ordinary PascalCase methods or only suppressing the pointer-identity fallback.
...
- [ ] Generator audit 22.18: close `.cswinrt` `write_abi_struct` parity for non-blittable structs; generated struct metadata must support field marshalers for string/object/interface/generic fields, `CreateMarshaler`, `FromAbi`, `FromManaged`, `CopyAbi`, `CopyManaged`, and `DisposeAbi` instead of only blittable scalar/nested layout.
- [ ] Generator audit 22.19: close `.cswinrt` `write_abi_delegate`, `get_generic_abi_types`, and `add_abi_delegates_for_type` parity; delegate parameters/returns must cover struct, GUID, mapped generic collection/async/reference shapes, and generated generic delegate invoke helpers without falling back to unsupported descriptor cases.
