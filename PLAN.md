...
- [ ] 24.8 正在做: dependency/wrapper closure: interface proxy generation now classifies enum/struct/delegate/interface/runtime-class members, skips setter-only supplemental properties, emits `IClosable.close()`, and avoids runtime-class wrapper/public-constructor signature collisions; remaining work is helper-only metadata for mapped XAML/DataTransfer/Direct3D surfaces.
...
- [ ] 24.10 正在做: ABI marshaler closure: `Map.Entry<K,V>`, `System.Object`/`IInspectableReference`, generated struct/value collection adapters, async object/delegate/enum readback, and collection marshaler lifetime now route through runtime/generator helpers; remaining work is nullable ABI pointers plus unresolved helper-only metadata return surfaces.
- [ ] 24.11 正在做: generic/interface helper initialization: generic delegate arity, non-shared generic event helper emission, interface proxy type classification, and delegate `System.Object` ABI support are tightened; remaining work is derived generic interface impl classes, mapped collection NativeProjection forwarding, and fallback helper registration.
- [ ] 24.12 正在做: mapped runtime-owned helper closure: finish `INotifyPropertyChanged`/`INotifyDataErrorInfo`, bindable helpers, and helper-only mapped metadata routing.
- [ ] 24.13 正在做: namespace additions/runtime support: finish non-authoring additions that Kotlin still needs from `.cswinrt/src/cswinrt/strings`, especially JSON `ToString`, Foundation collection helpers, Storage/DataTransfer adapters, Direct3D helper metadata, and unsigned type imports.
...
- [ ] `write_interface`, `write_interface_members`, `write_static_abi_classes`, `write_abi_interface`: interface shells, calls, native projection metadata, and non-collection native proxy typing exist; generic helper init, mapped collection helper routing, and metadata-only wrapper surfaces still need Queue 24.
- [ ] `write_class`, `write_static_class`, `write_class_members`, `write_static_members`: class shells and inherited/generic default-interface caches mostly exist; base constructor/object-reference routing, constructor conflict handling, and wrapper closure still need Queue 24.
- [ ] `write_custom_mapped_type_members` and required mapped helpers: collections and `IClosable` are partly aligned, including `Map.Entry` and inspectable collection adapters; bindable/data-error helper routing, helper-only metadata, struct collection adapters, and full Kotlin map/list interface completion still need Queue 24.
...
- [ ] `./.agent_scripts/run_windows_gradle.sh --no-configuration-cache :winrt-samples:run -Dkotlin.winrt.samples.runWinUiSmoke=true -PkotlinWinRt.samples.windowsAppSdkWinuiVersion=1.8.251105000 -PkotlinWinRt.samples.windowsAppSdkFoundationVersion=1.8.251104000 -PkotlinWinRt.samples.windowsAppSdkInteractiveExperiencesVersion=1.8.251104001` now clears constructor collisions, delegate `Metadata` generation for object-return callbacks, CoreWindow2 setter-only getter-slot emission, enum return line wrapping, generated struct collection adapter emission, and collection marshaler `.use` wrapping; it is still blocked by mapped collection NativeProjection forwarding, mapped XAML helpers, System.Object/Direct3D helper-only metadata, and JSON `ToString`.
