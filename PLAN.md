...
- [x] Generator default-interface contract safety: fail closed before rendering runtime-class default-interface surfaces when declared default interfaces are missing from the metadata model or do not carry metadata IIDs.
- [x] Generator implemented-interface contract safety: fail closed before rendering runtime-class implemented-interface surfaces when declared non-mapped implemented interfaces are missing from the metadata model or do not carry metadata IIDs.
- [x] Generator required-interface contract safety: fail closed before rendering required-interface forward/cache surfaces when declared non-mapped required interfaces are missing from the metadata model or do not carry metadata IIDs.
- [x] Generator event-delegate contract safety: fail closed before rendering event surfaces or EventSource helper support when non-mapped event delegates are missing from the metadata model or do not carry metadata IIDs.
- [x] Generator delegate ABI binding contract safety: fail closed before rendering method/property ABI bindings when delegate parameters or returns are missing delegate metadata IIDs required by delegate marshaling descriptors.
- [x] Generator projected runtime-class ABI binding contract safety: fail closed before rendering method/property ABI bindings when projected runtime-class parameters or returns are missing default-interface metadata IIDs required by projection marshalers.
- [x] Generator projected interface ABI binding contract safety: fail closed before rendering method/property ABI bindings when projected interface parameters or returns are missing metadata IIDs required by projection marshalers.
- [x] Generator unsupported ABI binding diagnostics: fail closed before rendering method/property ABI bindings when metadata classifies any parameter, return, or nested generic argument as an unsupported ABI shape.
- [x] Generator enum ABI binding contract safety: fail closed before rendering method/property ABI bindings when enum parameters, returns, or nested generic arguments are missing underlying type metadata required by ABI marshalers.
- [x] Generator struct ABI binding contract safety: fail closed before rendering method/property ABI bindings when struct parameters, returns, or nested generic arguments cannot produce native struct layout metadata required by ABI marshalers.
- [x] Generator delegate Invoke ABI binding contract safety: fail closed before rendering method/property ABI bindings when delegate Invoke parameters or returns require unsupported, incomplete enum, or incomplete struct ABI metadata.
- [x] Generator static property setter binding contract safety: fail closed before rendering static property projections when metadata declares a static setter but no matching static setter ABI binding can be resolved.
