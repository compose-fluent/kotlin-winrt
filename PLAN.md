...
- [ ] Authoring WinMD emission 正在做: authored runtime-class descriptors are owned by `winrt-metadata`, merge into the projection model before source generation, emit a deterministic descriptor file, and the PE/CLI WinMD writer now emits loadable TypeDef/base TypeRef/InterfaceImpl rows plus Default/Overridable/Activatable/Version custom attributes; next step is Gradle wiring for the actual `.winmd` artifact.
