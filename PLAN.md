...
- [ ] Authoring WinMD emission 正在做: authored runtime-class descriptors are now owned by `winrt-metadata`, merge into the projection model before source generation, emit a deterministic descriptor file, and the PE/CLI WinMD writer now emits loadable TypeDef/base TypeRef/InterfaceImpl rows plus Default/Overridable custom attributes; next step is Activatable/Version attributes and Gradle wiring for the actual `.winmd` artifact.
