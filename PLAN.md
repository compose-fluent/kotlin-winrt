...
- [ ] Create the generator surface aligned with `.cswinrt/src/cswinrt` while keeping Kotlin tests inside the owning modules (正在做: the initial metadata -> projection-plan -> Kotlin-text skeleton now enforces Generator 3.1 metadata contract guards for interface IIDs and static runtime-class activation metadata, so later generator work can build on metadata-derived contracts instead of handwritten projection files; next work is Generator 3.2 declaration planning.)
