...
- [ ] Generator ABI 8.20: split `.cswinrt` projected activation constructors from authoring/server activation generation: keep runtime-class `Activatable`/`Composable` projected constructors on the active generator path, but move `write_factory_class` and `write_module_activation_factory` server factory emission to the deferred authoring/native queue.
