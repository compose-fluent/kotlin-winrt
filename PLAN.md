...
- [ ] Create the `winrt-projections` module and wire deterministic generated output corresponding directly to `.cswinrt/src/Projections` (正在做: module is wired into the root Gradle build; `KotlinProjectionGeneratorTest` now carries a `Windows.Data.Json` generator regression fixture for the temporary smoke slice declaration set, and the current runtime-backed JSON files now share a `winrt-runtime` projection-support helper with generalized static-invocation wrappers so handwritten projection code no longer duplicates activation/static-call boilerplate while broader regeneration work catches up.)
