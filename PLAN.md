...
- [ ] 24.8 正在做: dependency/wrapper closure: generate metadata-only or wrapper-only surfaces for referenced excluded/root-filtered types instead of leaving unresolved `Metadata`, `wrap`, or `TYPE_HANDLE`.
...
- [ ] 24.11: generic/interface helper initialization: generic delegate arity and non-shared generic event helper emission are tightened; remaining work is derived generic interface impl classes, generic ABI helper init, and fallback helper registration.
