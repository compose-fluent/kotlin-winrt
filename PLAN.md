...
- [ ] Server/host activation 正在做: JVM host bridge mirrors `.cswinrt/src/Authoring` `DllGetActivationFactory`/`DllCanUnloadNow` HRESULT semantics, generated support registers module factories before forwarding, and library identity now publishes authored WinMD metadata; next is actual native DLL export/packaging.
