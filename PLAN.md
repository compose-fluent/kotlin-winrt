...
- [x] Completed authored CCW reachable ABI parity audit: aligned generator support with `.cswinrt/src/WinRT.Runtime/Projections/KeyValuePair.cs`, `.cswinrt/src/WinRT.Runtime/Projections/Nullable.cs`, `.cswinrt/src/WinRT.Runtime/ComWrappersSupport.cs`, and `.cswinrt/src/cswinrt/code_writers.h` for the generator-visible authored ABI surface. Overridable authored interfaces now marshal supported `IKeyValuePair<TKey,TValue>`, generic collections, bindable collection parameters and returns, async references, `IPropertyValue`, runtime-owned custom objects such as `Windows.Foundation.Uri`, and `IReference<T>` / `IReferenceArray<T>` through shared runtime projection helpers, and fail unsupported reachable shapes before support rendering.
