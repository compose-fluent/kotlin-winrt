...
- [ ] 24.11 正在做: generic/interface helper initialization: `.cswinrt` required-interface collection augmentation now feeds concrete NativeProjection/list forwarding and WindowsAppSDK sample generation; remaining work is true open-generic ABI helper/delegate classes for `T0` collection wrappers instead of suppressing incomplete open-generic NativeProjection.
...
- [ ] 24.13 正在做: namespace additions/runtime support: JSON `IStringable.ToString` now maps through Kotlin `toString`, generated support tables are chunked to avoid JVM `<clinit>` limits, and WindowsAppSDK sample compile/run passes; remaining work is Storage/DataTransfer adapters, Direct3D helper metadata, and unsigned cleanup.
...
- [ ] `write_interface`, `write_interface_members`, `write_static_abi_classes`, `write_abi_interface`: interface shells, calls, native projection metadata, non-collection native proxy typing, object-method mapping, and concrete required collection forwarding exist; open-generic ABI helper classes and metadata-only wrapper surfaces still need Queue 24.
...
- [ ] `write_custom_mapped_type_members` and required mapped helpers: collections and `IClosable` are partly aligned, including `Map.Entry`, inspectable adapters, concrete `IObservableVector<T>` return wrappers, and required list/map forwarding; bindable/data-error helper routing, helper-only metadata, struct collection adapters, and open-generic list/map ABI helpers still need Queue 24.
