...
- [ ] Authoring writer output 正在做: turn current plan files into actual generated wrapper/ABI/custom-QI/factory/module-activation support code; module activation output can register generated factories, runtime has the `WinRtActivationFactory` CCW contract, generator emits concrete per-runtime-class server activation factory shells, generated CCW factory registration emits interface method tables plus real event add/remove handlers, ordinary method/property bodies now cover the scalar/String/Guid/EventRegistrationToken/object-reference support set, and generated CCW definitions can forward custom QI to an existing `IWinRTObject.nativeObject`; next is widening marshaling coverage and wrapper/ABI class output.
