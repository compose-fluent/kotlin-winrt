...
- [ ] Manifest closure 正在做: validate packaged-mode `Applications/Application` presence, executable/entry-point consistency, app identity, visual elements when present, and fail-closed diagnostics before package creation; current slice closes pre-package validation for application presence, executable, entry point, and declared visual element assets.
- [ ] Package verification closure 正在做: add a cache-compatible package verification task using Windows packaging tools as evidence, and make it fail when package output is missing, malformed, or not installable enough for the current smoke target; current slice adds `verifyWinRtApplicationPackage` using `makeappx unpack` against generated appx/msix output.
