...
- [ ] Generator ABI 8.19d 正在做: finish `.cswinrt` `write_class_objrefs_definition` generic-interface initialization by replacing the current no-op `WinRTGenericTypeInstantiations.initializeDependencies(entry) { }` call with executable initialization that registers the generated generic RCW/vtable helpers before `As<IUnknownVftbl>(IID)`.
- [ ] Generator ABI 8.19: close `.cswinrt` object-reference cache rules from `write_class_objrefs_definition`; remaining work is only the executable generic-interface initialization path from 8.19d.
