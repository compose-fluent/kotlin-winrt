...
- [x] Completed authored CCW reachable ABI parity audit: aligned generator support with `.cswinrt/src/WinRT.Runtime/Projections/KeyValuePair.cs`, `.cswinrt/src/WinRT.Runtime/Projections/Nullable.cs`, `.cswinrt/src/WinRT.Runtime/ComWrappersSupport.cs`, and `.cswinrt/src/cswinrt/code_writers.h` for the generator-visible authored ABI surface. Overridable authored interfaces now marshal supported `IKeyValuePair<TKey,TValue>`, generic collections, bindable collections, async references, and `IReference<T>` / `IReferenceArray<T>` through shared runtime projection helpers, fail unsupported reachable shapes before support rendering, and leave `IPropertyValue` documented as runtime value-boxing/table support rather than a separate generator ABI shape until metadata modeling requires one.
...
- [x] Authored CCW pre-render contracts cover generator-known inherited/overridable ABI surfaces: supported `IKeyValuePair<TKey,TValue>`, generic collections, bindable collections, async references, and `IReference<T>` / `IReferenceArray<T>` decode/write through shared runtime projections, while unsupported reachable parameter or return shapes fail closed before `WinRTAuthoringCcwFactories` can emit unreachable handler bodies.
...
- [x] Inherited authored interface CCW coverage includes map-view/key-value-pair, generic/bindable collections, async references, and nullable/reference parity: `IMapView`-style `GetValueByIndex` returns are marshaled through the runtime key-value-pair collection host, supported collection/async/reference parameters are decoded before invoking authored Kotlin members, and unreachable unsupported shapes are diagnosed by the generator instead of downstream CCW scanning.
