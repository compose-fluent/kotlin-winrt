...
- [ ] 24.1: `write_interface` parity: emit/route interface `TYPE_HANDLE`, native projection metadata, generic/non-generic helper declarations, and wrapper-only metadata so generated wrappers do not reference missing `Metadata`/`wrap`.
- [ ] 24.2: `write_class` parity: fix base/default interface ownership, derived-class default-interface access, fast-ABI hierarchy dispatch, and base constructor/object-reference routing.
- [ ] 24.3: mapped runtime-owned helpers: align `IClosable`/`IDisposable`, `INotifyDataErrorInfo`, bindable helpers, and helper-only mapped types so generated code imports runtime helpers instead of skipped projection interfaces.
- [ ] 24.4: ABI marshaler nullability: align `IReference<T>`, object/interface/delegate returns, nullable ABI pointers, and adapter construction with `.cswinrt` `abi_marshaler`.
- [ ] 24.5: generic/interface helper initialization: align derived generic interface impl classes, generic ABI helper init, event generic dependencies, and fallback helper registration.
...
- [ ] 24.7: namespace additions/runtime support: finish non-authoring additions that Kotlin still needs from `.cswinrt/src/cswinrt/strings`, especially Foundation async helpers plus Storage/Streams adapters.
- [ ] 24.8: keep `write_wrapper_class`, `write_abi_class`, `write_factory_class`, `write_module_activation_factory`, CCW/server activation, and WinRT-exposed authoring output frozen under `winrt-authoring` until `mingwX64` prerequisites close.
...
- [ ] `write_interface`, `write_interface_members`, `write_static_abi_classes`, `write_abi_interface`: interface shells and calls mostly exist, but `TYPE_HANDLE`, native projection metadata, generic helper init, and metadata-only wrapper surfaces still need Queue 24.
- [ ] `write_class`, `write_static_class`, `write_class_members`, `write_static_members`: class shells mostly exist, but base/default interface access, fast-ABI hierarchy dispatch, and wrapper closure still need Queue 24.
- [ ] `write_custom_mapped_type_members` and required mapped helpers: collections are now aligned; `IClosable`, runtime-owned helpers, bindable/data-error helper routing, and helper-only metadata still need Queue 24.
...
- [ ] `./.agent_scripts/run_windows_gradle.sh --no-configuration-cache :winrt-samples:run -Dkotlin.winrt.samples.runWinUiSmoke=true -PkotlinWinRt.samples.windowsAppSdkWinuiVersion=1.8.251105000 -PkotlinWinRt.samples.windowsAppSdkFoundationVersion=1.8.251104000 -PkotlinWinRt.samples.windowsAppSdkInteractiveExperiencesVersion=1.8.251104001` now clears the removed XAML attribute include list and the mapped collection mutability/`Size`/`Remove` blockers; it is still blocked by Queue 24 items 24.1-24.7.
