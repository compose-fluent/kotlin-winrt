...
- [ ] Create the generator surface aligned with `.cswinrt/src/cswinrt` while keeping Kotlin tests inside the owning modules (正在做: the initial metadata -> projection-plan -> Kotlin-text skeleton now enforces Generator 3.1 metadata contract guards and completes Generator 3.2 declaration planning across interfaces, runtime classes, enums, structs, delegates, metadata/activation/static/composable companion surfaces, `.cswinrt`-aligned ownership traits, and declaration modifiers such as api-contract enum shape plus sealed/static class planning; next work moves to Generator 3.3 shell emission.)
