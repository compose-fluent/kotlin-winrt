...
- [ ] A11.1 Generic WinRT-exposed source scan 正在做: mirror cswinrt `WinRTExposedType` discovery for all Kotlin source types that implement WinRT interfaces or inherit WinRT runtime classes. Kotlin owner: plugin/source-generation plus `winrt-runtime` type-details lookup; `Application` is only the first validation case, not a special scan target.
- [ ] A11.2 Generated type-details lookup 正在做: mirror cswinrt `WinRTExposedTypeAttribute -> IWinRTExposedTypeDetails` lookup by letting JVM CCW creation discover generated `WinRT_*_TypeDetails` providers by source type name before falling back to synthetic `IInspectable`.
