...
- [ ] Manifest closure: validate packaged-mode `Applications/Application` presence, executable/entry-point consistency, app identity, visual elements when present, and fail-closed diagnostics before package creation; current baseline validates application presence, executable, entry point, declared visual element attributes, and staged package payload references for manifest executables/logos.
