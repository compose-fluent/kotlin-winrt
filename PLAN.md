...
- [ ] Queue 8: `winrt-projections` must regenerate the current temporary `Windows.Data.Json` smoke slice through the generator path before any broader checked-in projection surface is added.
...
- [ ] Metadata 2.1: replace placeholder modeling in `winrt-metadata` with real WinMD ingestion aligned with the metadata-reading responsibilities inside `.cswinrt/src/cswinrt`.
- [ ] Metadata 2.2: stabilize deterministic namespace/type normalization in `winrt-metadata` so the same metadata input always produces the same ordered model before generator breadth expands.
- [ ] Metadata 2.3: add core shape fidelity next: kind classification, GUID/default-interface data, base/implemented interface relationships, generic parameter shapes, and runtime-class activation/static/factory metadata.
- [ ] Metadata 2.4: add member/signature fidelity next: method/property/event descriptions, parameter direction/pass-by semantics, and the normalized signature inputs needed by the generator.
...
- [ ] Generator 3.1: keep the Kotlin generator behind completed Metadata 2.1 through Metadata 2.4 prerequisites and derive its behavior from `.cswinrt/src/cswinrt` responsibilities rather than from existing handwritten projection files.
- [ ] Generator 3.2: implement declaration planning first: determine the Kotlin declaration set for enums, structs, delegates, interfaces, runtime classes, and metadata companions before detailed member emission grows.
- [ ] Generator 3.3: implement deterministic declaration-shell emission next: namespace layout, type declarations, interface/runtime-class scaffolding, companion metadata surfaces, and stable file/text ordering.
- [ ] Generator 3.4: implement member emission next: methods, properties, events, interface inheritance handling, default-interface/runtime-class composition, and activation/static/factory surfaces.
- [ ] Generator 3.5: only after Generator 3.1 through Generator 3.4 are coherent, add special-case rules for collections, async, custom type mappings, and WinUI-specific projection behavior.
- [ ] Generator gate: the current `KotlinProjectionGenerator` skeleton and handwritten `Windows.Data.Json` projection files are transitional scaffolding; do not treat them as permission to expand `winrt-projections` before the metadata and generator prerequisites are closed.
...
- [ ] Sample 6.1: keep `winrt-samples` on the smallest smoke scenario for the currently completed slice first; the current JSON smoke surface is a narrow validation target, not a signal to broaden sample scope.
...
- [ ] Create the `winrt-metadata` module and land the JVM metadata foundation aligned with the metadata-loading responsibilities inside `.cswinrt/src/cswinrt` (正在做: this starts after the runtime foundation needed by the active slice is clear; type/namespace model now has deterministic normalization plus initial method/parameter/kind modeling, but real WinMD loading is still missing)
...
- [ ] Keep `winrt-metadata` in ingestion-first then fidelity-second order (正在做: the module currently has a thin normalized model, so the next work must go to real WinMD loading and symbol-shape completeness rather than richer projection helpers)
- [ ] Create the generator surface aligned with `.cswinrt/src/cswinrt` while keeping Kotlin tests inside the owning modules (正在做: established an initial metadata -> projection-plan -> Kotlin-text skeleton for interface/runtime-class declarations; this remains behind the active runtime and metadata prerequisites, and further hand-written projection expansion should stop here and move into generator rules derived from `.cswinrt` instead of test-shaped backfilling)
...
- [ ] Freeze handwritten projection growth until the existing generator skeleton can reproduce the current `Windows.Data.Json` smoke slice through the planned metadata/generator path
...
- [ ] Create the `winrt-projections` module and wire deterministic generated output corresponding directly to `.cswinrt/src/Projections` (正在做: module is wired into the root Gradle build, but it is not the starting point for new implementation slices; the checked-in `Windows.Data.Json` slice remains only as a temporary runtime smoke surface, while deterministic output must move through the generator skeleton after the required runtime and metadata prerequisites are in place)
...
- [ ] Treat the current handwritten `Windows.Data.Json` files as a temporary comparison target, not as the design source for future generator rules
- [ ] Create the `winrt-samples` module and wire the JVM sample validation surface corresponding directly to `.cswinrt/src/Samples` (正在做: module is now wired into the root Gradle build and already contains a real WinRT JSON smoke sample on top of `winrt-runtime`; samples validate implemented runtime/generator behavior and must not become the place where missing design is invented)
- [ ] Keep `winrt-samples` on smoke-first validation only until the upstream runtime/generator/authoring prerequisites are ready (正在做: the current module has a narrow JSON smoke surface and should not expand into broader WinUI or hosting scenarios until the upstream contracts exist)
