...
- [ ] Kotlin appx/msix packaging completeness 正在做: finish manifest generation/validation, appx/msix layout, dependency payload resolution, resources/PRI/MRT, signing/test-install hooks, unpackaged vs packaged modes, and Gradle DSL ergonomics without cloning full MSBuild; current slice makes explicit signing and test-install failures fail closed.
- [ ] Manifest closure: validate packaged-mode `Applications/Application` presence, executable/entry-point consistency, app identity, visual elements when present, and fail-closed diagnostics before package creation; current baseline validates application presence, executable, entry point, and declared visual element assets.
- [ ] Package verification closure: add a cache-compatible package verification task using Windows packaging tools as evidence, and make it fail when package output is missing, malformed, or not installable enough for the current smoke target; current baseline adds `verifyWinRtApplicationPackage` using `makeappx unpack` against generated appx/msix output.
...
- [ ] Gradle cache contract 正在做: keep configuration cache and build cache enabled for WinRT generation and packaging tasks; cache failures are issues to fix, not flags to bypass. Application package staging is cacheable with output-affecting resource-root path modeled as an input; runtime asset staging now snapshots explicit `runtimeAsset(...)` files, dependency-identity runtime asset files, already-present NuGet package content roots, and a runtime NuGet package-root manifest produced by a dedicated resolver task. NuGet restore / CLI-discovered package content still needs to be fully removed from the staging task action before the task can be safely marked cacheable.
