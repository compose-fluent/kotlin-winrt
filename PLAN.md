...
- [ ] Generator ABI 8.22: implement `.cswinrt` event source helper subclass parity for generated event surfaces: `write_event_source_subclass`, `write_event_source_table`, and `EventHandler<T>` generic initialization must move from passive `WinRTEventProjectionHelpers` metadata to callable runtime/helper code.
