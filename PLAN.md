...
- [ ] Payload/resource closure 正在做: keep explicit package payload staging, resource inventories, PRI/MRT generation, dependency layout, makepri inputs, package-root path sensitivity, absolute/escaping path rejection, and caller/manifest-field-specific relative-path diagnostics deterministic and owned by the Kotlin Gradle plugin.
