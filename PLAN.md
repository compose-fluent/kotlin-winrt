...
- [ ] Authoring WinMD emission 正在做: Kotlin authored runtime-class candidates now merge into the projection metadata model before source generation, matching `.cswinrt` `GenerateWinMD -> GenerateSources` ordering; next step is replacing the in-memory model handoff with a real WinMD/PE metadata writer.
