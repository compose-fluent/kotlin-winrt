...
- [ ] 24.8 正在做: dependency/wrapper closure: interface proxy generation now uses metadata type classification for enum/struct/delegate/interface/runtime-class members, skips mapped collection proxy inheritance, and emits `IClosable.close()` for supported proxies; remaining work is metadata-only surfaces for unresolved `Metadata`/`TYPE_HANDLE` and excluded wrapper references.
...
- [ ] 24.10 正在做: ABI marshaler closure: `Map.Entry<K,V>` and `System.Object`/`IInspectableReference` collection adapters now route through runtime adapters; remaining work is non-string `IReference<T>`, struct/value adapters such as `TextSegment`, object/interface/delegate returns, nullable ABI pointers, and collection return values.
- [ ] 24.11: generic/interface helper initialization: generic delegate arity, non-shared generic event helper emission, and interface proxy type classification are tightened; remaining work is derived generic interface impl classes, generic ABI helper init, mapped collection proxy suppression, and fallback helper registration.
- [ ] 24.12: mapped runtime-owned helper closure: finish `INotifyDataErrorInfo`, bindable helpers, and helper-only mapped metadata routing.
- [ ] 24.13: namespace additions/runtime support: finish non-authoring additions that Kotlin still needs from `.cswinrt/src/cswinrt/strings`, especially Foundation async helpers plus Storage/Streams adapters and unsigned type imports.
