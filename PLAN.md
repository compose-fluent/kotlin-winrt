...
- [ ] Signing and install closure: keep signtool and PowerShell install/test-install inputs provider-modeled, fail closed on missing tools or invalid packages, skip missing install inputs when install is disabled, and preserve disabled task outputs without deleting previous artifacts.
