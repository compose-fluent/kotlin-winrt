...
- [ ] Server/host activation 正在做: JVM host bridge mirrors `.cswinrt/src/Authoring` `DllGetActivationFactory`/`DllCanUnloadNow` HRESULT semantics, generated support registers module factories before forwarding, library identity publishes authored WinMD metadata, and applications stage own/dependency authored WinMD beside runtime assets; next is actual native DLL export/packaging.
