...
- [ ] Server/host activation 正在做: JVM host bridge mirrors `.cswinrt/src/Authoring` `DllGetActivationFactory`/`DllCanUnloadNow` HRESULT semantics, generated support exposes stable host-shim address entrypoints, library identity/staging handles authored WinMD and host manifests, and authoring can install staged host manifests as activation-factory fallbacks; next is actual native DLL export/packaging.
