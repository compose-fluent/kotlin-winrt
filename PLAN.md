...
- [x] Generator fail-closed baseline: current generator rejects missing or incomplete metadata for composable factories, activation/static interfaces, default/implemented/required interfaces, event delegates, delegate/runtime-class/interface ABI bindings, unsupported ABI shapes, enum/struct/array ABI metadata, delegate `Invoke` ABI metadata, and static setter ABI bindings before rendering.
