...
- [ ] Create the `winrt-metadata` module and land the JVM metadata foundation aligned with the metadata-loading responsibilities inside `.cswinrt/src/cswinrt` (正在做: Metadata 2.1 through Metadata 2.4 are now closed with a JVM-side PE/CLI metadata reader, canonical path deduplication, deterministic normalization, core type-shape fidelity, and member/signature modeling for methods, properties, and events; current work has moved from row-id preservation into `.cswinrt`-style ABI-semantics fidelity by carrying enum underlying integral types and enum members forward so generator-side `abi_marshaler` planning can be metadata-driven instead of signature/name-driven.)
...
- [ ] Create the generator surface aligned with `.cswinrt/src/cswinrt` while keeping Kotlin tests inside the owning modules (正在做: the metadata -> projection-plan -> KotlinPoet generator now enforces Generator 3.1 metadata contract guards, completes Generator 3.2 declaration planning, completes Generator 3.3 deterministic shell emission, completes Generator 3.4 member emission, and completes Generator 3.5 special-case type mapping from `.cswinrt/src/cswinrt/helpers.h` for collections, async, custom mapped types, and WinUI bindable surfaces; current work is no longer expanding ad hoc binder branches and instead is tightening generator-side `abi_marshaler` parity so instance/static member binding is driven by metadata type semantics, enum ABI readback/writeback is emitted inline, delegate shells now require exactly one metadata `Invoke` contract, emit runtime-backed `Metadata.fromAbi(...)` wrappers, consume runtime delegate descriptors/references for delegate-return readback, and now project `.cswinrt`-style read-only collection interface surfaces for `IIterable<T>`, `IVectorView<T>`, and `IMapView<K,V>` onto runtime-class `Iterable` / `List` / `Map` delegates backed by owner-interface ABI slots while reusing that same collection marshaling path for mapped method/property returns. `IVector<T>` and `IMap<K,V>` now also project to runtime-class and returned `MutableList<T>` / `MutableMap<K,V>` wrappers via the same inline ABI path. The next unfrozen generator slice is collection-parameter parity and the remaining bindable collection ABI categories, while struct/interface-proxy ABI categories still fail explicitly instead of being misclassified as projected objects.)
...
- [ ] Create the `winrt-projections` module and wire deterministic generated output corresponding directly to `.cswinrt/src/Projections` (正在做: module is wired into the root Gradle build; `KotlinProjectionGeneratorTest` carries a `Windows.Data.Json` generator regression fixture for a representative standard WinRT API declaration set, and the current work is to move generated output toward real runtime-backed consumption by emitting runtime-binding descriptors, companion-level `ActivationFactory`/static-interface accessors, interface-owned ABI slot constants, runtime-class RCW shells with internal inspectable wrapping, cached objref accessors for implemented interfaces, runtime-member owner descriptors that map instance members and static-interface members onto the correct objref/slot-owner pair, a minimal `IWinRTObject/nativeObject` bridge for projected-object parameter marshalling, and a `.cswinrt`-style generator-side `abi_marshaler` model whose inline `invokeAbi` emission is now metadata/type-semantics driven. Delegate shells now require exactly one metadata `Invoke` contract, emit runtime-backed `Metadata.fromAbi(...)` wrappers, and support delegate-return readback through runtime delegate references; unit-return outbound delegate parameters/events stay on the existing bridge path, and runtime classes that inherit `IIterable<T>`, `IVectorView<T>`, or `IMapView<K,V>` now emit `Iterable` / `List` / `Map` delegation surfaces backed by owner-interface ABI slots, iterator current/move-next calls, and key/value-pair readback, with mapped method/property returns reusing that same collection marshaling path instead of falling back to raw WinRT members. `IVector<T>` and `IMap<K,V>` now emit `MutableList<T>` / `MutableMap<K,V>` runtime surfaces and mapped return wrappers through the same inline ABI emitter. The next projection-growth slice is collection-parameter parity and the remaining bindable collection ABI categories. Struct member marshalling and interface-proxy ABI remain frozen behind explicit generator errors until upstream contracts exist.)
