...
- [ ] Authoring WinMD emission 正在做: authored runtime-class descriptors are now owned by `winrt-metadata`, merge into the projection model before source generation, and emit a deterministic authoring metadata descriptor file; next step is replacing the descriptor writer with the real WinMD/PE metadata writer matching `.cswinrt/src/Authoring/WinRT.SourceGenerator/WinRTTypeWriter.cs`.
