...
- [ ] Generator audit boundary 正在做: the full non-authoring `.cswinrt/src/cswinrt` generator scan is represented by completed 22.x plus 23.1-23.5. Remaining sample-run blockers are base/default interface visibility, mapped `IClosable`/runtime-owned helpers, interface type-handle metadata, reference-nullability, and wrapper metadata/wrap exposure.
...
- [ ] `./.agent_scripts/run_windows_gradle.sh --no-configuration-cache :winrt-samples:run -Dkotlin.winrt.samples.runWinUiSmoke=true -PkotlinWinRt.samples.windowsAppSdkWinuiVersion=1.8.251105000 -PkotlinWinRt.samples.windowsAppSdkFoundationVersion=1.8.251104000 -PkotlinWinRt.samples.windowsAppSdkInteractiveExperiencesVersion=1.8.251104001` now clears the removed XAML attribute include list and the mapped collection mutability/`Size`/`Remove` blockers; it is still blocked by base/default interface visibility, mapped `IClosable`/runtime-owned helpers, interface type-handle metadata, reference-nullability, and wrapper metadata/wrap exposure.
