...
- [ ] Create the generator surface aligned with `.cswinrt/src/cswinrt` while keeping Kotlin tests inside the owning modules (正在做: the initial metadata -> projection-plan -> KotlinPoet shell generator now enforces Generator 3.1 metadata contract guards, completes Generator 3.2 declaration planning, and completes Generator 3.3 deterministic shell emission across specialized interface/runtime-class/static-class/attribute/api-contract builders plus metadata/activation/static/composable companion shells; next work moves to Generator 3.4 member emission.)
