...
- [ ] Create the `winrt-metadata` module and land the JVM metadata foundation aligned with the metadata-loading responsibilities inside `.cswinrt/src/cswinrt` (正在做: Metadata 2.1 and Metadata 2.2 are now closed with a JVM-side PE/CLI metadata reader plus canonical path deduplication and deterministic namespace/type/method normalization that mirrors the `.cswinrt/src/cswinrt` cache/namespaces discovery baseline; next work is Metadata 2.3/2.4 shape fidelity.)
...
- [ ] Keep `winrt-metadata` in ingestion-first then fidelity-second order (正在做: real file ingestion and deterministic normalization are now in place, so the next work must stay on symbol-shape completeness rather than richer projection helpers)
