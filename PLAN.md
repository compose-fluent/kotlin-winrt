...
- [x] Inherited authored interface CCW coverage includes map-view/key-value-pair, generic/bindable collections, async references, `IPropertyValue`, and nullable/reference parity: `IMapView`-style `GetValueByIndex` returns are marshaled through the runtime key-value-pair collection host, supported collection/async/property-value/reference parameters are decoded before invoking authored Kotlin members, and unreachable unsupported shapes are diagnosed by the generator instead of downstream CCW scanning.
