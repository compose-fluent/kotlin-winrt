...
- [ ] Generator audit 22.10 正在做: align HRESULT handling with `.cswinrt` `is_noexcept` and `write_abi_method_call_marshalers`; generated vtable calls currently call `HResult(__hr).requireSuccess()` uniformly instead of suppressing throws for `NoExceptionAttribute` and remove overloads.
