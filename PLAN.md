...
- [ ] Gradle cache contract 正在做: keep configuration cache and build cache enabled for WinRT generation and packaging tasks; cache failures are issues to fix, not flags to bypass. Application package staging is cacheable with output-affecting resource-root path modeled as an input; runtime asset staging now snapshots explicit `runtimeAsset(...)` files, dependency-identity runtime asset files, and already-present NuGet package content roots, and resolves staging from modeled package roots when the full package closure is present. NuGet restore / CLI-discovered package content still needs to be split out before the task can be safely marked cacheable.
