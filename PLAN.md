...
- [ ] Align collection projection for iterable/list/map/view adapters (正在做: `winrt-runtime` now has collection interface signature/IID helpers for `IIterable`/`IIterator`/`IVector`/`IVectorView`/`IMap`/`IMapView`/`IKeyValuePair`, plus initial `IIterable`/`IIterator`/`IVectorView`/`IMap`/`IMapView` reference wrappers and slot-level unit coverage, but split/IndexOf/GetMany and broader projected collection behavior are still missing)
