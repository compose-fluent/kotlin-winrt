...
- [ ] Generator audit 22.11 正在做: align runtime-class identity with `.cswinrt` `write_class`; generated runtime classes implement `IWinRTObject` but still need pointer-based `equals`/`hashCode` semantics matching cswinrt `IEquatable`/operator equality.
